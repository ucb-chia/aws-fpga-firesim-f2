 ...
 * 2. fpga_mgmt_load_local_image_sync_flags: Load the AFI to the fpga using the prefetch option. This operation will
 *    load the specified AFI into the AFI cache on the FPGA image slot. The AFI is not loaded to the FPGA. Performing
 *    the asynchronous load operation (fpga_mgmt_load_local_image_flags) with large uncached AFIs is not recommended as
 *    one or more FPGA image slots may require a retry.
 * 3. fpga_mgmt_load_local_image: After the AFI is cached on each FPGA, cards can be loaded with the AFI in
 *    parallel without the need to retry.
 * 4. Poll: Poll using fpga_mgmt_describe_local_image until the slots are no longer busy or perform other operations
 *    while the FPGA load is completing.
 * 5. fpga_pci_rescan_slot_app_pfs: Rescan the pci bus to allow the SDK to pick up any changes in vendor id or device
 *    id.
 * 6. Display and Verify: Display and verify the contents returned in the fpga_mgmt_image_info struct.
...
int main(int argc, char ** argv) {
  ...
  for (size_t i = 0; i < slot_id_count; ++i) {
    ...
    printf("Loading FPGA image slot %d with prefetch\n", slot_id);
    struct fpga_mgmt_image_info info;
    ret = fpga_mgmt_load_local_image_sync_flags(slot_id, afi_image, FPGA_CMD_PREFETCH, num_retries, retry_delay, &info);
  ...
  /* 5. fpga_pci_rescan_slot_app_pfs: */
