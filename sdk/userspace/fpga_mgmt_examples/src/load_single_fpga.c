...
/*
 * This example demonstrates how to load an AFI to a single FPGA image slot using the C API.
 ...
 * 2. fpga_mgmt_load_local_image_sync: Load the AFI to the fpga. This operation will load the specified FPGA
 *    image to the FPGA image slot.
...
int main(int argc, char ** argv) {
  ...
  int slot_id = 0;
  /* Number of internal calls to fpga_mgmt_describe_local_image attempted by the fpga_mgmt_load_local_image_sync function. */
  uint32_t num_retries = 30000;
  /* Delay in milliseconds between internal calls to fpga_mgmt_describe_local_image by the fpga_mgmt_load_local_image_sync function. */
  ...
  /* 2. fpga_mgmt_load_local_image_sync: */
  printf("Loading FPGA image slot %d with AFI %s\n", slot_id, afi_image);
  struct fpga_mgmt_image_info info;
  ret = fpga_mgmt_load_local_image_sync(slot_id, afi_image, num_retries, retry_delay, &info);
  fail_on(ret != 0, err, "fpga_mgmt_load_local_image_sync failed. An incorrect slot_id or a lower num_retries can cause this call to fail.");
