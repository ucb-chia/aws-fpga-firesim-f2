...
2. Run `cd hdk/cl/examples` and `./create_new_cl.py --new_cl_name $NEW_CL_NAME`
3. Run `cd $NEW_CL_NAME` and `export CL_DIR=$PWD`
4. Place all design files into `$CL_DIR/design` (overwriting the existing ${NEW_CL_NAME}.sv file)
5. Simulate with `cd $CL_DIR/verif/scripts and make ${NEW_CL_NAME}_base_test <SIMULATOR>=1`
...
Or specify an [available simulator](./../../../../User_Guide_AWS_EC2_FPGA_Development_Kit.md#hardware-development-kit-hdk):
